    ...
    <div class="col-sm-12">
        <div class="login-panel panel panel-info">
            <div class="panel-heading">
                <div class="row">
                    <div class="col-xs-12 col-md-12 col-lg-12 text-left"><strong>修改问卷调查</strong></div>
                </div>
            </div>
            <div class="panel-body">
                ...
                <div class="box box-default">
                    <form id="survey_save" class="form-horizontal" method="post" action="poll/update"
                          enctype="multipart/form-data" target="nm_iframe">
                        <input type="hidden" value="${pollCreateVo.id }" name="id"/>
                                ...
                                <div class="col-xs-12 col-lg-12">
                                    ...
                                    <div class="input-group date ml10" style="width:175px;float: left;">
                                        <input type="text"
                                               ...
                                               name="datetimeStart" id="datetimeStart"
                                               value="${pollCreateVo.datetimeStart }" readonly UNSELECTABLE="on">
                                    ...
                                    <div class="input-group date" style="width:175px;">
                                        <input type="text"
                                               style="display: block;width: 100%;line-height: 20px;padding: 6px 12px;border: 1px solid #ccc;"
                                               name="datetimeEnd" id="datetimeEnd" value="${pollCreateVo.datetimeEnd }"
                                ...
                                <div class="col-xs-8 col-lg-7 ml-40">
                                    <div class="col-sm-12 single-topic-title-discribe">
                                        <textarea id="survey_title" name="title" class="form-control" rows="1"
                                                  placeholder="请输入本次问卷的标题">${pollCreateVo.title }</textarea>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-xs-8 col-lg-7 ml-40">
                                    <div class="col-sm-12 single-topic-title-discribe">
                                        <textarea id="survey_des" name="des" class="form-control" rows="3"
                                                  placeholder="请输入本次问卷的目的描述">${pollCreateVo.des }</textarea>
                                    </div>
                                </div>
                            </div>


                            <c:forEach items="${pollCreateVo.surveyTopicVos }" var="topics" varStatus="index">
                            <div class="row pb20 pt20 mlr-10 border-top-1">
                                <button type="button" onclick="removeTopicDiv(this)" class="btn btn-box-tool"><i
                                        class="fa fa-times"></i></button>
                                <div class="col-xs-4 col-lg-4 single-topic-title"><input type="hidden"
                                                                                         name="surveyTopicVos[${index.index }].topicTypeNum"
                                                                                         value="${topics.topicTypeNum}">
                                </div>
                                <div class="col-xs-8 col-lg-8 ml-40 single-topic-answer-list">
                                    <div class="">
                                        <div class="answer-list-default" style="padding-left: 20px;">
                                            <div class="list-wrap mt15" style="margin-left: 18px;">
                                                <span name="num_s">Q${index.index+1 }:</span>
                                                <input name="surveyTopicVos[${index.index }].opts_title" type="text"
                                                       value="${topics.opts_title }">&nbsp;
                                                <c:if test="${topics.topicTypeNum == 0 }">(单选)</c:if>
                                                <c:if test="${topics.topicTypeNum == 1 }">(多选)</c:if>
                                                <c:if test="${topics.topicTypeNum == 2 }">(填空)</c:if>
                                            </div>
                                        </div>
                                        <div class="answer-list-default">
                                            <c:if test="${topics.topicTypeNum == 2 }">
                                                <textarea readonly="readonly" class="form-control" rows="3"
                                                          style="margin-top: 10px;width: 400px;margin-left: 70px;"></textarea>
                                            </c:if>

                                            <c:if test="${topics.topicTypeNum == 0 || topics.topicTypeNum == 1}">
                                            <c:forEach items="${topics.surveyOptVos }" var="opt" varStatus="opt_index">
                                            <c:if test="${opt.ans == 1 }">
                                            <div class="list-wrap mt15 active">
                                                <i name="opt_c" style="display:none;" onclick="check_focus(this)" class="fa fa-check-circle">
                                                    <input style="display: none;"
                                                           name="surveyTopicVos[${index.index }].surveyOptVos[${opt_index.index }].ans"
                                                           value="${opt.ans }">
                                                </i>
                                                </c:if>

                                                <c:if test="${opt.ans == 0 }">
                                                <div class="list-wrap mt15 ">
                                                    <i name="opt_c" style="display:none;" onclick="check_focus(this)" class="fa fa-circle-o">
                                                        <input style="display: none;"
                                                               name="surveyTopicVos[${index.index }].surveyOptVos[${opt_index.index }].ans"
                                                               value="${opt.ans }">
                                                    </i>
                                                    ...
                                                    <span name="num_s" style="display: block;float: left;width: 65px;line-height: 40px;text-align: right;">选项${opt_index.index+1 }:</span>
                                                    <input name="surveyTopicVos[${index.index }].surveyOptVos[${opt_index.index }].optValue"
                                                           type="text" value="${opt.optValue }">
                                                    <a href="javascript:void(0);" style="margin-left: 5px;">
                                                        <i class="glyphicon glyphicon-picture">
                                                            <input type="file" onclick="opt_change_file(this)"
                                                                   name="surveyTopicVos[${index.index }].surveyOptVos[${opt_index.index }].myfile"
                                                                   style="position: absolute;opacity: 0;z-index: 1;top: -15px;left: -20px;width:1px;"/>
                                                        </i>
                                                    </a>
                                                    <c:if test="${topics.topicTypeNum == 0}">
                                                        <c:if test="${opt_index.index > 1}">
                                                            <b style="position: relative;z-index: 2;"
                                                               onclick="remove_opt(this)">-</b>
                                                        </c:if>
                                                    </c:if>
                                                    <c:if test="${topics.topicTypeNum == 1}">
                                                        <c:if test="${opt_index.index > 2}">
                                                            <b style="position: relative;z-index: 2;"
                                                               onclick="remove_opt(this)">-</b>
                                                        </c:if>
                                                    </c:if>
                                                    <div style="margin-left: 80px;margin-top: 5px;">
                                                        <c:if test='${empty opt.picSrc}'>
                                                            <img style="display: none;" width="120" height="212"
                                                                 src="static/images/cp_05.jpg"/>
                                                            <i onclick="remove_img(this)"
                                                               class="glyphicon glyphicon-remove"
                                                               style="display: none;"></i>
                                                        </c:if>
                                                        <c:if test='${not empty opt.picSrc}'>
                                                            <img style="display: block;" width="120" height="212"
                                                                 src="${opt.picSrc}"/>
                                                            <i onclick="remove_img(this)"
                                                               class="glyphicon glyphicon-remove"
                                                               style="display: block;position: relative;top: -210px;left: 100px;"></i>
                                                        </c:if>
                                                        <input style="display: none;"
                                                               name="surveyTopicVos[${index.index }].surveyOptVos[${opt_index.index }].picSrc"
                                                               value="${opt.picSrc}">
                                                    </div>
                                                </div>
                                                </c:forEach>
                                    ...
                                    <c:if test="${topics.topicTypeNum == 0 || topics.topicTypeNum == 1}">
                                        <div class="col-xs-7 col-lg-7 col-xs-offset-4 col-lg-offset-4 mt20">
                                            <a href="javascript:void(0);" onclick="add_opt_line(this)"
                                               class="mybtn btn-add-user2"><i class="fa fa-plus"></i>添加选项</a>
                                        </div>
                                    </c:if>
                                </div>
                                </c:forEach>
...
<script type="text/javascript">
    var topicCount = 0;
    $(function () {
    	var count_topic = $("#page_content").children().length;
    	topicCount = parseInt(count_topic,10) - 3;
    ...
    function removeTopicDiv(obj) {
        ...
        console.log(topicNum+"--"+topicCount);
    ...
    function remove_img(obj) {
        ...
        //清空file文件
        var input_files = $(ob).children().children();
