		...
		<div class="row">
			...
			<div class="col-xs-6 col-md-6 col-lg-6 text-right">
				<ol class="list-inline" style="padding:10px 15px;">
		  			<li><button type="button" id="publishBtn" onclick="editRecruit()" class="btn btn-primary btn-default" style="width:80px;border-radius:15px;">发布</button></li>
				</ol>	
			</div>
		</div>
...
<!-- 			<input type="text" name="jobId" value="0" style="display:none;"/> -->
			...
			success:function(data){
				if(data!=undefined && data!='undefined'){
					...
					if(data.files && data.files.length>0){
						existFilesCount = data.files.length;
						for(var m=0; m<data.files.length; m++){
							$("#fileSpan").append('<span style="margin-right:15px;">'+data.files[m].fileName+'<i class="glyphicon glyphicon-remove" style="color:red;" onclick="delExistedFile(this, '+data.files[m].aId+')"></i></span>');
						}
					}
	...
	//打开文件上传对话框
	function addFile(){
		if(filesCount+existFilesCount>3){
	...
	function delExistedFile(obj, id){
		layer.confirm('删除之后服务器文件也将删除，确认删除？', {
		    btn: ['确定','取消'], //按钮
		    shade: false //不显示遮罩
		}, function(index1){
			$(obj).parent().remove();
